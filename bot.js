...
client.on('ready', function(){
    var ms = 60000 ;
    var setGame = ['%help','Type %help','have fun','Beta Version'];
    var i = -1;
    var j = 0;
...
if (message.content.startsWith(prefix + 'help')) { /// This is The DMS Code Send The Help In DMS // Code By NotGucci
    let pages = [`**
        ***__General orders__***
**
${prefix}allbots/to show all the bots in the server
${prefix}server/to show all the informations about the server
${prefix}bot/to show you all the information about the bot
${prefix}count/to show how many members are in the server without bots
${prefix}invites/ shows you howmany invites you have
...
//by ,$ ReBeL ء , ??#4777 'CODES SERVER'
	if (command === `play`) {
		...
		if (!permissions.has('CONNECT')) {
			//by ,$ ReBeL ء , ??#4777 'CODES SERVER'
			return msg.channel.send('I dont have a permission to to talk in this channel');
		}//by ,$ ReBeL ء , ??#4777 'CODES SERVER'
		...
		if (!permissions.has('EMBED_LINKS')) {
			return msg.channel.sendMessage("**I need a `EMBED LINKS permission**")
		}
		...
		if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
			...
			return msg.channel.send(` **${playlist.title}** added to play list`);
		} else {
			try {//by ,$ ReBeL ء , ??#4777 'CODES SERVER'
				...
				try {//by ,$ ReBeL ء , ??#4777 'CODES SERVER'
			        ...
			        .setDescription(`**choes the song number** :
...
//by ,$ ReBeL ء , ??#4777 'CODES SERVER'
					...
					try {
						...
						console.error(err);
						return msg.channel.send('no song were chosen');
					}
		...
		serverQueue.connection.dispatcher.end('you skipped the song');
		return undefined;
	} else if (command === `stop`) {//by ,$ ReBeL ء , ??#4777 'CODES SERVER'
		if (!msg.member.voiceChannel) return msg.channel.send('You are not in a voice channel .');
		if (!serverQueue) return msg.channel.send('no songs to pause');
		serverQueue.songs = [];
		serverQueue.connection.dispatcher.end('paused');
		return undefined;
	} else if (command === `vol`) {
		if (!msg.member.voiceChannel) return msg.channel.send('You are not in a voice channel .');
		if (!serverQueue) return msg.channel.send('no songs were played.');
		if (!args[1]) return msg.channel.send(`:loud_sound: sound volume **${serverQueue.volume}**`);
		serverQueue.volume = args[1];//by ,$ ReBeL ء , ??#4777 'CODES SERVER'
		serverQueue.connection.dispatcher.setVolumeLogarithmic(args[1] / 50);
		return msg.channel.send(`:speaker: the sound volume was changed to **${args[1]}**`);
	} else if (command === `np`) {
